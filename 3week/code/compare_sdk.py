...
def call_with_sdk() -> None:
    ...
    #
    # 📌 최신 ollama 패키지는 순수 dict 가 아니라 ollama._types.ChatResponse 객체를
    #    돌려준다(딕셔너리처럼 [] 접근은 그대로 됨). 예전 버전·블로그 예제는 dict 였다.
    #    → 핵심 논지는 같다: "공급자마다 응답 구조가 제각각이고, 그 구조를 외워야 한다"
    #
    # ⚠️ 역할 이름 오타({"role": "sytem"})는 KeyError 가 아니다.
    #    role 이 그냥 문자열이라 에러 없이 그대로 실행된다 — 틀린 줄도 모른다.
...
관찰 — 무엇이 같고 무엇이 다른가
  ...
  반환 타입     ChatResponse ([] 로 꺼냄)      AIMessage
  메시지 표현   딕셔너리                       타입 객체
  역할 오타     "sytem" → 에러 없이 실행       SystemMesage → 에디터가 잡음
  꺼내기 오타   resp["mesage"] → KeyError      .content 하나 (자동완성)
